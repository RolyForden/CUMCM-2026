# 会话交接

> 交叉验证曾发现结构缺陷，现已完成整改。人类已在D003正式选择时间方案A；N1–N3仍待人类裁决。

## 项目事实

- 项目已经选择 C 题，阶段为 `SOLVING`。
- D002已冻结13项P0/P1/P2口径；D003已冻结N4为方案A；N1–N3仍为人类裁决项。
- 整改前探针提交为 `2250523`，原 `experiments/probe_timing.md` 的 41/41 保留为历史基线，不删除、不改写。
- 交叉验证发现：字典序等式、全年固定读第 2 行、执行器重写弃光、核算空集/SOC 断链、T4 连续循环、规则法终端补费、简化模板冒充官方模板等结构缺陷。
- 整改代码提交为 `7464ccca35ad5a0eb454d0441fb6e051264e1582`。

## 整改结果

- 已同步 `DECISIONS.md` 的实施记录、`research/C_method_map.md` 的模型符号与基线要求，以及历史任务/报告的失效标记；D002不变，N4按人类指令写入D003。
- 主探针 `src/probe_timing.py`：48/48，通过。
- 独立交叉验证 `src/probe_cross_validation.py`：39/39，通过。
- Q1 双引擎费用：`35126.948589390` 元；历史锚点仍在容差内。
- 时间边界对照：E_144=E_0 为 `35126.948590`；E_143=E_0 为 `34881.454457`；差异 `245.494132` 元（四舍五入与旧记录末位差异不影响判断）。
- 官方 `result1.xlsm` 只在系统临时目录复制、原位填写和回读；工作表顺序、隐藏状态、非目标单元格与 VBA 容器检查通过。
- 证据：`research/C_probe_cross_validation.md`、`experiments/probe_cross_validation_results.json`。

## 当前裁决

- Q1的LP—执行—核算—模板链，在D002参数和D003方案A下重新认证为可信；35126.948590元为正式主口径结果。
- Q1定稿验证已完成：附件与CSV逐槽复算误差不超过`1.82e-12`量级；显式互斥MILP费用差`7.28e-12`元；相对无储能节省`26.8981%`；效率是最敏感参数（费用约±3.8%）。正式CSV未修改。
- Q1 正式结果独立交叉验证 `src/verify_q1_result.py`：32/32 通过（独立 MILP 路径费用 35126.948589290 元，与 LP 差 1e-7 元；产物逐槽复算、模板重开回读、外部锚点一致）；证据 `experiments/q1_cross_validation_report.md`、`experiments/q1_cross_validation_results.json`。
- Q1 正文已迁入论文模板：已验证 latex 模板迁至 `paper/`（含字体），内容写入 `paper/数模通用模板.tex`（摘要问题一/问题重述/假设/符号/Q1 节/检验/文件列表），`paper/q1_section.md` 已删除；xelatex 编译通过（14 页无错误）。
- 禁止进入 Q2–Q4 全年正式运行。
- N1阻塞Q2执行器；N2阻塞Q3多次调整账本；N3阻塞附件3首小时预测适配。N4已解除。

## 下一步

1. ~~按方案A生成Q1正式结果文件，并做模板回读与独立核算。~~ **已完成（2026-09-12）**：
   - `src/generate_q1_result.py`：LP 双引擎求解 → 执行器 → 独立核算 → 官方 `result1.xlsm` 原位填写（计划购电量 144 槽 + 按题面补齐“充放电量”表，官方原有工作表/隐藏表/VBA 保持不变）→ 独立重开核验（读 CSV 复算残差/SOC/费用 + 重开 xlsm 回读两表 + SHA-256 对照）。
   - 产物 `outputs/q1_scheme_a/`：`result1.xlsm`、`q1_dispatch.csv`、`q1_paper_tables.json`（论文表 1/表 2）、`q1_audit.json`（16 项生成期检查 + 8 项重开检查，全过）。
   - 主口径费用 **35126.948589390 元**；`probe_timing.py` 48/48、`probe_cross_validation.py` 39/39 复跑无回归。
2. ~~系统验证 Q1 并完成论文正文。~~ **已完成（2026-09-12）**：
   - 新增 `src/validate_q1.py` 和最小单元测试；结果位于 `outputs/q1_validation/`，图位于 `figures/q1_*.png`。
   - 验证报告 `research/C_q1_validation.md`；论文正文 `paper/q1_section.md`。
   - 当前环境 Python 3.11.2 / SciPy 1.16.1 仍复现正式费用，旧 48/39 探针无回归。
3. ~~人类裁决N1–N3后，只先做对应小窗口和账本反例，不直接跑全年。~~ **论文模板迁移与 Q1 入 tex（2026-09-12）**：已验证 latex 模板迁至 `paper/`（含字体），Q1 正文写入 `paper/数模通用模板.tex`（摘要/问题重述/假设/符号/Q1 节/检验/文件列表），`paper/q1_section.md` 已删除；xelatex 编译通过（14 页无错误），PDF 数值抽查全部命中。
4. 后续代码继续使用 `BatteryParams`、分离的因果数据接口和独立核算器，不恢复旧全局耦合。

## 不要重复或回退

- 不要恢复 `TOPIC_SELECTION`、未选题或未编写代码等旧状态。
- 不要把成员个人桌面路径写入共享文档。
- 不要用 `research/C_method_map_cross_validation.md` 作为探针代码通过证据；它只保留方法地图历史审查定位。
- 不要覆盖 `data/raw/`，也不要提交模板测试产生的临时二进制文件。
